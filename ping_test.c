...
#define SO_RECV_TRAFFIC_CLASS 0x1087
#define SO_RECV_ANYIF 0x1104
#define SO_TC_CTL 900
...
int main(int argc, char *const *argv) {
    ...
    while ((ch = getopt(argc, argv, "Aab:Cc:DdfG:g:h:I:i:k:Ll:M:m:nop:QqRrS:s:T:t:vW:z:")) != -1) {
        switch (ch) {
            case 'A':
                options |= F_MISSED;
                break;
    ...
    while (!finish_up) {
        ...
        if (n == 1) {
            ...
            // pr_pack((char *)packet, cc, &from, &recv_time, tc);
        ...
        if (n == 0 || options & F_FLOOD) {
            ...
            // printf("1 Request timeout for icmp_seq %ld\n", ntransmitted - 2);
            ...
            // printf("2 Request timeout for icmp_seq %ld\n", ntransmitted - 2);
            (void)gettimeofday(&last, NULL);

            if (ntransmitted - nreceived - 1 > nmissedmax) {
                // printf("%ld %ld %ld\n", ntransmitted, nreceived, nmissedmax);
